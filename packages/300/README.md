...
This package provides the Google [Material Symbols](https://fonts.google.com/icons?icon.set=Material+Symbols) with weight 300 converted to [`SvgIcon`](https://mui.com/material-ui/api/svg-icon/) components.
...
```bash
npm install @mui-symbols-material/w300
```
<!-- #default-branch-switch -->

To install icons of a different weight, simply replace `300` with the desired weight in the installation command.
...
import { 
  ...
  FavoriteFilledSharp,
} from '@mui-symbols-material/w300'; // Replace 300 with your desired weight
